...
def connect_existing_db(db_full_name_path):
    ...
    try:
        ...
        rdconfig.config_status['db_obj'].append(conn) 
    except sqlite3.Error as err:
...
def add_entry_command(c_entry):
    ...
    """
    if rdconfig.config_status['login'] == 'N':
        print('You are not logged in.')
        return False
    else:
        # Get the object
        conn = rdconfig.config_status['db_obj'][0]
        nc = conn.cursor()
...
def login_db(db_name):
    ...
    if rdconfig.config_status['login'] == 'Y':
        print(f'You have logged in already. Accessing database {rdconfig.config_status["database_access"]}')
        return True
    
    # Three attempts
    try_count = 0 
    while try_count < 3 and rdconfig.config_status['login'] == 'N':
        ...
        if validate_hashed_key(master_key, hashed_key):
            ...
            rdconfig.config_status['login'] = 'Y'
            rdconfig.config_status['database_access'] = db_name
            print(f'Login success. Accessing database - {db_name}:')

            # if the master key is correct, hash it to the config_status
            rdconfig.config_status['gen_key'] = sha_mod_key(master_key.encode())
...
def logout_db():
    ...
    """
    rdconfig.config_status['login'] = 'N'
    rdconfig.config_status['database_access'] = ''
    rdconfig.config_status['db_obj'] = []
